...
**Our tooling status:** `ads_mcp/creation/` does not yet support AI Max (no create/propose path, no MCP tool). Building that (a `search.py` extension or a dedicated builder plus a `propose_/commit_` pair, reading `GOOGLE_ADS_API_REFERENCE.md` first) is the implementation follow-up once we commit to running AI Max campaigns. Until then, AI Max is enabled via the web UI, and this skill governs the settings.

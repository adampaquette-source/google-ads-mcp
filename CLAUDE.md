...
| Control center UI or commit flow | `control_center/app.py` + `control_center/templates/` + `CONTROL_CENTER_SPEC.md`. Redeploy after change (local installer AND `railway up --service ads-control-center` for the hosted copy). Auth/read-only gating lives in `control_center/webauth.py`. |
